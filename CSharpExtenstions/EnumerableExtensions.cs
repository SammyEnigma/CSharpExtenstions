...
namespace CSharpExtenstions
{
    public static class EnumerableExtensions
    {
        ...
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="source">New list items</param>
        /// <param name="destination">Existing list items</param>
        /// <param name="onInsert">Insert functionality when entity is not present in destination</param>
        /// <param name="onDelete">Delete functionality when the entity is not present in source</param>
        /// <param name="onUpdate">Update funcationality when entity is present in both source and destination</param>
        public static void SyncLists<T, TKey>(this IEnumerable<T> source, IEnumerable<T> destination, Func<T, TKey> idSelector, Action<T> onInsert, Action<T> onDelete, Action<T> onUpdate)
        {

            if (destination.IsNullOrEmpty())
            {
                source.Each(s => onInsert(s));
                return;
            }
            ...
            if (!source.IsNullOrEmpty())
            {
                source.Each(s =>
                {
                    if (idSelector(s).Equals(default(TKey)))
                        onInsert(s);
                });

                var sourceDictionary = source.ToDictionary(idSelector);

                destination.Each(d =>
                {
                    var destId = idSelector(d);

                    if (!sourceDictionary.ContainsKey(destId))
                        onDelete(d);
                });
            }
